...
import {searchImportAuthDepart, searchImportVenues,getAuthorNameById} from "@/api/author";


const indicatorOptions = [{
      value: 'pi',
      label: 'pi(the P-index with equal A-index of this author)'
    }, {
      value: 'hi',
      label: 'hi(the H-index of this author)'
    }, {
      value: 'upi',
      label: 'upi(the P-index with unequal A-index of this author)'
    }]
  ...
  methods: {
    ...
    drawImportAuthDept(data,authorLimit){
      ...
      if (chart) {
        ...
        // 作者名称
        let nameArr = []
        data.importAuthors.forEach((item)=>{
          nameArr.push(item.name)
        })
        //hi最大值
        let hiMax = data.importAuthors.reduce((result,item)=>{
          return result < parseInt(item.hi) ? parseInt(item.hi) : result
        },0)
        //pc最大值
        let pcMax = data.importAuthors.reduce((result,item)=>{
          return result < parseInt(item.pc) ? parseInt(item.pc) : result
        },0)
        //pi最大值
        let piMax = data.importAuthors.reduce((result,item)=>{
          return result < parseFloat(item.pi) ? parseFloat(item.pi) : result
        },0)
        //cn最大值
        let cnMax = data.importAuthors.reduce((result,item) => {
          return result < parseInt(item.cn) ? parseFloat(item.cn) : result
        },0)
        // upi最大值
        let upiMax = data.importAuthors.reduce((result,item) => {
          return result < parseFloat(item.upi) ? parseFloat(item.upi) : result
        },0)
        //具体数据
        let circleData = []
        data.importAuthors.forEach((item,index) => {
      ...
      this.$refs['ruleFormAuthorDepart'].validate((valid) => {
        if (valid) {
          ...
          let param = {
            ...
            indicator: this.ruleFormAuthorDepart.indicator,
